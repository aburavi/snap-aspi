...
func TransactionHistoryList(ctx context.Context, req *history.TransactionHistoryListRequest) (*history.TransactionHistoryListResponse, error) {
	...
	if serr != nil {
		...
		data.ResponseCode = "5001201"
		data.ResponseMessage = fmt.Sprintf("Internal Server Error. %s", serr)
		//derr := status.Errorf(5001201, data.ResponseMessage)
		return &data, nil
	}
	...
	fmt.Printf("request history list: " + string(sdbody))
	...
	if reqBodyHash != bodyGenHash {
		...
		data.ResponseCode = "4012100"
		data.ResponseMessage = "Unauthorized. Signature body not matched"
		//derr := status.Errorf(4012100, data.ResponseMessage)
		return &data, nil
	}
...
func TransactionHistoryDetail(ctx context.Context, req *history.TransactionHistoryDetailRequest) (*history.TransactionHistoryDetailResponse, error) {
	...
	if serr != nil {
		fmt.Println("error wrong body format.")
		data.ResponseCode = "5001301"
		data.ResponseMessage = fmt.Sprintf("Internal Server Error. %s", serr)
		//derr := status.Errorf(5001301, data.ResponseMessage)
		return &data, nil
	}
