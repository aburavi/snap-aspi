...
func BalanceInquiry(ctx context.Context, req *inquiry.BalanceInquiryRequest) (*inquiry.BalanceInquiryResponse, error) {
	...
	fmt.Println("channelid: " + dchannel)
	...
	intergrpcrequest, dderr := GrpcValidasi(dpth, grpcheader.Token, grpcheader.Signature, clientid, extid, reffid)
	if dderr != nil {
	...
	fmt.Println("Signature: " + intergrpcrequest.Signature)
	fmt.Println("hashbody: " + bodyGenHash)
	fmt.Println("reqBodyHash: " + reqBodyHash)
...
func InternalAccountInquiry(ctx context.Context, req *inquiry.InternalAccountInquiryRequest) (*inquiry.InternalAccountInquiryResponse, error) {
	...
	// POST:/api/v1.0/balance-inquiry:muhpwhwOkPRU9nNXYnyYHj8t54x3:8b4e9e83b5231cff4f84358ec8ca81951cfe9f999f635b1566452a501d5c23b2:2021-11-29T09:22:18.172+07:00
	sbody, serr := json.Marshal(req)
	fmt.Println("sbody: ", string(sbody))
	if serr != nil {
		fmt.Println("error wrong body format.")
	...
	if grpcheader.XPartnerid != clientid {
		...
		data.ResponseMessage = fmt.Sprintf("Unauthorized. clientid not matched")
		//derr := status.Errorf(4011500, data.ResponseMessage)
		//st := status.New(codes.NotFound, data.ResponseMessage)
		//sta, _ := st.WithDetails(&data)
		return &data, nil
	}
	//Check Alphanumeric
	checkString := regexp.MustCompile(`\d+`).MatchString(req.PartnerReferenceNo)
	...
	checkBeneficiaryNo := regexp.MustCompile("[A-Z, a-z]+").MatchString(req.BeneficiaryAccountNo)
	if checkBeneficiaryNo {
		fmt.Println("Error when validation mandatory request")
		data.ResponseCode = "4001501"
		data.ResponseMessage = "Invalid field format, BeneficiaryAccountNo Not Allowed"
		//derr := status.Errorf(4001500, data.ResponseMessage)
		return &data, nil
	}
	...
	if dderr != nil {
		if strings.Contains(dderr.Error(), "decryptionerror") {
			...
			data.ResponseMessage = fmt.Sprintf("Conflict Unique externalid today")
			//dderr := status.Errorf(4091500, data.ResponseMessage)
			return &data, nil
		} else {
			fmt.Println("Error when validation request: %s", dderr)
			data.ResponseCode = "4001500"
	...
	b := dresp.Body()
	fmt.Printf("internaldata code=%d. [err=%v]\n", dresp.StatusCode(), string(b))
	if dresp.StatusCode() != fasthttp.StatusCreated {
	...
	if data.BeneficiaryAccountStatus == "not found" {
		...
		data.ResponseMessage = fmt.Sprintf("Bad Account")
		data.ReferenceNo = ""
		data.PartnerReferenceNo = ""
		data.BeneficiaryAccountNo = ""
		data.BeneficiaryAccountStatus = ""
	...
	//data.ResponseCode = "2001500"
	//data.ResponseMessage = "Request has been processed successfully"
	//data.BeneficiaryAccountStatus = "Rekening aktif"
...
func ExternalAccountInquiry(ctx context.Context, req *inquiry.ExternalAccountInquiryRequest) (*inquiry.ExternalAccountInquiryResponse, error) {
	...
	if derr1 != nil {
		...
		data.ResponseCode = "5001602"
		data.ResponseMessage = fmt.Sprintf("External Server Error. %s", derr1)
		//derr := status.Errorf(5001602, data.ResponseMessage)
		return &data, nil
	}
